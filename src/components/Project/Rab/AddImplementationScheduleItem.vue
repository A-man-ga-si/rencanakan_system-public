<template>
  <b-modal
    ...
    <div class="form-group">
      ...
      <p class="text-disabled">{{ itemName }}</p>
    </div>
    <div class="form-group">
      <label class="font-weight-bold">Bobot</label>
      <p class="text-disabled">{{ weight }} %</p>
    </div>
    <div class="implementation-range-group">
      <p class="font-weight-bold text-center w-100">
        Range Pengerjaan
      </p>
      <div class="implementation-range-input-group" v-for="(weekData, i) in weeksChoice" :key="i">
        <div class="row">
          <div :class="{'col-md-6': i >= 1, 'col': i < 1}">
            <b-form-group label="Minggu dimulai" class="has-float-label">
              <v-select label="label" v-model="weekData.start" :reduce="week => week.id" :options="formattedWeeksDataSelector" />
            </b-form-group>
          </div>
          <div :class="{'col-md-5': i >= 1, 'col': i < 1}">
            <b-form-group label="Minggu selesai" class="has-float-label">
              <v-select label="label" v-model="weekData.end" :reduce="week => week.id" :options="formattedWeeksDataSelector" />
            </b-form-group>
          </div>
          <div v-if="i > 0">
      ...
      <b-alert variant="danger" show class="rounded" size="xs" v-if="correctionAlert">
        <PhInfo class="mb-1 mr-1"/>
        Terdapat minggu yang saling overlap atau tidak sesuai urutan
      </b-alert>
      <div class="text-center">
        <b-button variant="outline-primary" @click="addRange">+ Tambah Range</b-button>
      </div>
    </div>
      <template slot="modal-footer">
        <b-button @click.prevent="submit" variant="primary" class="mr-1" :disabled="correctionAlert">
          {{ $t('modal.save-bt') }}
        </b-button>
    ...
    checkForCorrection() {
      ...
      for (const weeksChoice of this.weeksChoice) {
        if ((weeksChoice.start && weeksChoice.end)) {
          if (reservedWeeks.includes(weeksChoice.start) || reservedWeeks.includes(weeksChoice.end) || weeksChoice.end < weeksChoice.start) {
            invalid = true
          }
    ...
    async submit() {
      const data = await this.updateImplementationSchedule({
        ...
        implementationScheduleData: {
          ...
          rab_item_id: this.rabItemHashid,
        }
      })
