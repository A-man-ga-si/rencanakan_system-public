    ...
    <div class="talent-card-wrapper">
      <div class="talent-card">
        <div class="talent-image-container">
          <img class="talent-image" :src="user.image" :alt="user.name" />
          <div class="bookmark-icon" @click="$emit('toggle-bookmark', user)">
            <i class="simple-icon-star" :class="{ 'filled': user.bookmarked }"></i>
          </div>
          <div class="talent-badge experience-badge" v-if="user.experience">
            {{ user.experience }} Tahun Pengalaman
          </div>
        ...
        <div class="talent-info">
          <h3 class="talent-name">{{ user.name }}</h3>
          <p class="talent-location" v-if="user.location">
            <i class="simple-icon-location-pin"></i> {{ user.location }}
          </p>
