    ...
    <div class="d-flex flex-grow-1 min-width-zero">
      ...
      >
        <div class="list-item-heading mb-0 truncate w-80 mb-1 mt-1">
          <span class="heading-number d-inline-block">{{ sort }}</span
          >{{ title }}
        </div>
      </div>
      ...
      <div class="custom-control custom-checkbox pl-1 align-self-center pr-4">
        ...
        <b-button
          variant="outline-theme-3"
          class="icon-button rotate-icon-click rotate"
        ...
        >
          <i
            :class="{
              ...
              'simple-icon-arrow-down': !showDetail,
              'simple-icon-arrow-up': showDetail,
            }"
          ></i>
        </b-button>
      </div>
      ...
      <div class="card-body pt-0">
          ...
          <div class="form-group" v-if="answerType.options">
            ...
            >
              ...
              >
                ...
                <div class="input-icons">
                  <span class="badge badge-pill handle pr-0 mr-0"
                    ><i class="simple-icon-cursor-move"
                  /></span>
                  <span class="badge badge-pill" @click="deleteAnswer(a.value)"
                    ><i class="simple-icon-ban"
                  /></span>
                </div>
              </div>
            </draggable>
            ...
            <div class="text-center">
              <b-button
                ...
                class="mb-2"
                @click="addAnswer"
                ><i class="simple-icon-plus btn-group-icon" /> Add
                Answer</b-button
              >
            </div>
          </div>
        ...
        <div v-else-if="mode == 'preview'" key="preview">
          ...
          <v-select
            ...
            v-else-if="answerType.value == 1"
          />
          <v-select
            :options="answers"
            key="multiple-select-preview"
            v-else-if="answerType.value == 2"
          />
          <b-form-checkbox-group
            stacked
            key="checkbox-preview"
            v-else-if="answerType.value == 3"
          >
            <b-form-checkbox
              :value="a.value"
              v-for="(a, index) in answers"
              :key="`ans_${index}`"
              >{{ a.label }}</b-form-checkbox
            >
          </b-form-checkbox-group>
          <b-form-radio-group
            stacked
            key="radiobutton-preview"
            v-else-if="answerType.value == 4"
          >
            <b-form-radio
              :value="a.value"
              v-for="(a, index) in answers"
              :key="`ans_${index}`"
              >{{ a.label }}</b-form-radio
            >
          </b-form-radio-group>
        </div>
      </div>
