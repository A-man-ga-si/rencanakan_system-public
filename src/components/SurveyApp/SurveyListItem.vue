<template>
  <b-card
    @click="toggleItem($event, data.id)"
    :class="{ 'd-flex mb-3': true, active: selectedItems.includes(data.id) }"
    no-body
  >
    <div class="d-flex flex-grow-1 min-width-zero">
      ...
      >
        <router-link
          :to="`${adminRoot}/applications/survey/${data.id}`"
          class="list-item-heading mb-0 truncate w-40 w-xs-100 mb-1 mt-1"
        >
          <i
            :class="{
              'heading-icon': true,
        ...
        <div class="w-15 w-xs-100">
          <b-badge :variant="data.labelColor" pill>{{ data.label }}</b-badge>
        </div>
      </b-card-body>
      <div
        class="
          ...
          custom-control custom-checkbox
          pl-1
          align-self-md-center align-self-start
          pt-4
        "
      >
        <b-form-checkbox
